...
public class BankTransactionAmountPrompt extends SimplePrompt {
    BaseBank bank;
    BaseAccount promptAccount;
    boolean isDeposit;

    public BankTransactionAmountPrompt(final BaseAccount baseAccount, final BaseBank bank, final boolean isDeposit) {
        super(false);

        this.promptAccount = baseAccount;
    ...
    Prompt acceptValidatedInput(@NotNull final ConversationContext context, @NotNull final String input) {
        ...
        if (isDeposit) {
            ...
            bank.deposit(promptAccount, transactionQuantity);
        } else {
            BalanceUtil.giveBalance(getPlayer(context), transactionQuantity);
            bank.withdraw(promptAccount, transactionQuantity);
        }
    ...
    public void onConversationEnd(final SimpleConversation conversation, final ConversationAbandonedEvent event) {
        ...
        tell(player, "Your have successfully " + (isDeposit ? "deposit" : "withdraw") + " your money in the bank vault!");
