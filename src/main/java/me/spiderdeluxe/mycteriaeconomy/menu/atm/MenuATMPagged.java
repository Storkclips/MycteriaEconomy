 ...
 */
public abstract class MenuATMPagged<T> extends Menu {
     ...
     */
    protected MenuATMPagged(final Iterable<T> pages) {
        this(null, pages);
    }
     ...
     */
    protected MenuATMPagged(final Menu parent, final Iterable<T> pages) {
     ...
     */
    @Deprecated
    protected MenuATMPagged(final int pageSize, final Iterable<T> pages) {
     ...
     */
    @Deprecated
    protected MenuATMPagged(final int pageSize, final Menu parent, final Iterable<T> pages) {
     ...
     */
    @Deprecated
    protected MenuATMPagged(final int pageSize, final Menu parent, final Iterable<T> pages, final boolean returnMakesNewInstance) {
     ...
     */
    private MenuATMPagged(final Integer pageSize, final Menu parent, final Iterable<T> pages, final boolean returnMakesNewInstance) {
    ...
    public static <T> Map<Integer, List<T>> fillPages(int cellSize, final Iterable<T> items) {
        ...
        switch (cellSize) {
            case 9 -> cellSize = 9 - 2;
            case 18 -> cellSize = 18 - 4;
            case 27 -> cellSize = 27 - 6;
            case 36 -> cellSize = 36 - 8;
        }
    ...
    public ItemStack getItemAt(final int slot) {
        ...
        switch (slot) {
            case 10 -> {
                if (getCurrentPageItems().size() >= 1)
                    object = getCurrentPageItems().get(0);
                else
                    return emptyItemButton.getItem();
            }
            case 11 -> {
                if (getCurrentPageItems().size() >= 2)
                    object = getCurrentPageItems().get(1);
                else
                    return emptyItemButton.getItem();
            }
            case 12 -> {
                if (getCurrentPageItems().size() >= 3)
                    object = getCurrentPageItems().get(2);
                else
                    return emptyItemButton.getItem();
            }
            case 13 -> {
                if (getCurrentPageItems().size() >= 4)
                    object = getCurrentPageItems().get(3);
                else
                    return emptyItemButton.getItem();
            }
            case 14 -> {
                if (getCurrentPageItems().size() >= 5)
                    object = getCurrentPageItems().get(4);
                else
                    return emptyItemButton.getItem();
            }
        }
    ...
    public final void onMenuClick(final Player player, final int slot, final InventoryAction action, final ClickType click, final ItemStack cursor, final ItemStack clicked, final boolean cancelled) {
        ...
        switch (slot) {
            case 10 -> {
                if (getCurrentPageItems().size() >= 1)
                    obj = getCurrentPageItems().get(0);
            }
            case 11 -> {
                if (getCurrentPageItems().size() >= 2)
                    obj = getCurrentPageItems().get(1);
            }
            case 12 -> {
                if (getCurrentPageItems().size() >= 3)
                    obj = getCurrentPageItems().get(2);
            }
            case 13 -> {
                if (getCurrentPageItems().size() >= 4)
                    obj = getCurrentPageItems().get(3);
            }
            case 14 -> {
                if (getCurrentPageItems().size() >= 5)
                    obj = getCurrentPageItems().get(4);
            }
        }
        if (obj != null) {
