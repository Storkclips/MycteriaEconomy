 ...
 */
public class NPCBase implements ConfigSerializable {
    ...
    public void createNPC(final Location loc, final String modelName) throws NPCLoadException {
        ...
        final NPCStorage npcStorage = NPCStorage.getInstance();
        npcStorage.addNPC(npcBase);
    ...
    public SerializedMap serialize() {
        ...
        map.put("EntityType", getType());
        map.put("Citizen", getCitizen().getUniqueId());
        map.put("Citizen_ID", getCitizen().getId());
        map.put("Function", getFunction());

        if (getShopName() != null)
            map.put("Shop_Name", getShopName());
        if (getAtmUUID() != null)
            map.put("ATM_Uuid", getAtmUUID());
        return map;
    }

    public static NPCBase deserialize(final SerializedMap map) {

        final UUID uuid = map.getUUID("UUID");
        final String name = map.getString("Name");
        final EntityType entityType = EntityType.valueOf(map.getString("EntityType"));

        NPC citizen = CitizensAPI.getNPCRegistry().getByUniqueId(map.getUUID("Citizen"));
        if (citizen == null)
            citizen = CitizensAPI.getNPCRegistry().getById(map.getInteger("Citizen_ID"));

        final NPCFunction function = NPCFunction.fromName(map.getString("Function"));


        switch (function) {
            case SHOP -> {
                final String shopName = map.getString("Shop_Name");
                return new NPCShop(uuid, name, shopName, entityType, citizen);
            }
            case ATM -> {
                final UUID atmUUID = map.getUUID("ATM_Uuid");
                if (atmUUID.equals(uuid)) {
                    return new NPCAtm(name, atmUUID, entityType, citizen);
                }
            }
            case TELLER -> {
                return new NPCTeller(name, uuid, entityType, citizen);
            }
        }
        return null;
    }
