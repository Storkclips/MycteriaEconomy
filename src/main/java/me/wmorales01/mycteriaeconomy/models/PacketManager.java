...
public class PacketManager {
    ...
    public void uninjectPacket(Player player) {
        if (!channels.containsKey(player.getUniqueId())) return;
    ...
    private void readPacket(Player player, Packet<?> packet) {
        ...
        if (getValue(packet, "action").toString().equalsIgnoreCase("INTERACT")) {
            ...
            for (AbstractNPCShop npcShop : plugin.getNpcShops().values()) {
                EntityPlayer entityPlayer = npcShop.getEntityPlayer();
                if (npcShop.getEntityPlayer().getId() != id) continue;

                Bukkit.getScheduler().runTaskLater(plugin,
                        () -> Bukkit.getPluginManager().callEvent(new NPCRightClickEvent(player, entityPlayer)), 0L);
            }
        }
    }
